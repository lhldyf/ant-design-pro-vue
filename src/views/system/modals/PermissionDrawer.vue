      ...
      <a-spin :spinning="confirmLoading">
        <a-form :form="form">
          ...
          <a-form-item v-show="!show" :labelCol="labelCol" :wrapperCol="wrapperCol" label="授权策略">
            <a-radio-group v-decorator="['permsType', {}]">
              <a-radio key="1" value="1">可见/可访问</a-radio>
              <a-radio key="2" value="2">可编辑</a-radio>
            </a-radio-group>
          </a-form-item>
          <a-form-item v-show="!show" :labelCol="labelCol" :wrapperCol="wrapperCol" label="状态">
            <a-radio-group v-decorator="['permsType', {}]">
              <a-radio key="1" value="1">有效</a-radio>
              <a-radio key="0" value="0">无效</a-radio>
            </a-radio-group>
          </a-form-item>
...
export default {
  ...
  data () {
    return {
      ...
      initFields: ['name', 'perms', 'permsType', 'component', 'url', 'sortNo', 'menuType', 'status']
    }
  },
  props: {
    treeData: Array
  ...
  methods: {
    ...
    beforEdit (record) {
      ...
      if (record.isRoute != null) {
        this.routeSwitch = !!record.isRoute
      }
    ...
    handleOk () {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          let obj
          if (!this.model.id) {
            obj = postAction(that.url.create, formData)
          } else {
            obj = postAction(that.url.update, formData)
          }
          obj
            .then(res => {
              if (res.success) {

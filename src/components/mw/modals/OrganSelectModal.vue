<template>
  ...
  >
    <a-spin tip="Loading..." :spinning="false">
      <a-input-search style="margin-bottom: 1px" placeholder="请输入单位名称按回车进行搜索" @search="onSearch" />
      <a-tree
        checkable
        :treeData="treeData"
        :checkStrictly="true"
...
export default {
  ...
  methods: {
    ...
    initOrganComponent () {
      let names = ''
      if (this.organId) {
        const currOrganId = this.organId
    ...
    onCheck (checkedKeys, info) {
      if (!this.multi) {
        const arr = checkedKeys.checked.filter(item => {
